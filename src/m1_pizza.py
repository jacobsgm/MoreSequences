...
def run_test_generate_points_on_circle():
    ...
    # Test 2:
    expected2 = [rg.Point(75.0, 50.0),  # All numbers are approximate.
                rg.Point(62.5, 71.7),
                rg.Point(37.5, 71.7),
                rg.Point(25.0, 50.0),
                rg.Point(37.5, 28.3),
    ...
    answer2 = generate_points_on_circle(circle, 6)
...
def run_test_draw_points_on_circle():
    ...
    # Test 4:
    title = 'DRAW_POINTS_ON_CIRCLE, test 4:  666 red dots.'
    window = rg.RoseWindow(1000, 1000, title)
    circle = rg.Circle(rg.Point(200, 200), 100)
    draw_points_on_circle(window, circle, 666, 'red')
    window.close_on_mouse_click()
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    circle.attach_to(window)
    for k in range(len(i)):
        newline = rg.Line(i[k], circle.center)
        newline.color = color
...
def run_test_polygon():
    ...
    title = 'POLYGON test 3:  18 segments with very thick red lines;'
    window = rg.RoseWindow(550, 400, title)

    circle = rg.Circle(rg.Point(100, 100), 80)
    circle.outline_thickness = 3
    polygon(window, circle, 18, 'red', 15)
    window.continue_on_mouse_click()
